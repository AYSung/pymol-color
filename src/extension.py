...
def import_moda(path: Path) -> pd.DataFrame:
    ...
    return (
        ...
        .assign(
            label=lambda x: pd.cut(
                x.label, bins=bins, labels=labels, include_lowest=True
            ),
...
def import_gnomad(path: Path) -> pd.DataFrame:
    ...
    return (
        ...
        .drop(columns='type')
        .assign(
            residue=lambda x: x.residue.str.extract(r'(\d+)'),
            label=lambda x: x.label.str.lower()
            .str.replace(' ', '_')
...
def bin_residues(data: pd.DataFrame) -> Tuple[str, str, str]:
    ...
    """
    return (
        data.astype({'residue': str})
        .groupby(['label', 'color'], observed=True, sort=False)
        .agg('+'.join)
